...
public extension UIView {
    func showWaitOverlay(color: UIColor) {
        self.isUserInteractionEnabled = false
        _ = SwiftOverlays.showCenteredWaitOverlay(parentView: self, color: color)
    }
    
    func removeAllOverlays() -> Void  {
        self.isUserInteractionEnabled = true
        SwiftOverlays.removeAllOverlaysFromView(parentView: self)
    }
}
...
public extension UIViewController {
    ...
    func showWaitOverlay() {
        showWaitOverlay(color: UIColor.white)
    }
    
    func showWaitOverlay(color: UIColor) {
        self.view.isUserInteractionEnabled = false
        _ = SwiftOverlays.showCenteredWaitOverlay(parentView: self.view, color: color)
    }
    ...
    func showWaitOverlayWithText(text: NSString) -> UIView  {
        self.view.isUserInteractionEnabled = false
        return SwiftOverlays.showCenteredWaitOverlayWithText(parentView: self.view, text: text)
    }
    ...
    func showTextOverlay(text: NSString) -> UIView  {
        self.view.isUserInteractionEnabled = false
        return SwiftOverlays.showTextOverlay(parentView: self.view, text: text)
    }
    ...
    func showProgressOverlay(text: NSString) -> UIView  {
        return SwiftOverlays.showProgressOverlay(parentView: self.view, text: text)
    }
    ...
    func showImageAndTextOverlay(image: UIImage, text: NSString) -> UIView  {
        return SwiftOverlays.showImageAndTextOverlay(parentView: self.view, image: image, text: text)
    }
     ...
     */
    class func showNotificationOnTopOfStatusBar(notificationView: UIView, duration: TimeInterval, animated: Bool = true) {
        SwiftOverlays.showAnnoyingNotificationOnTopOfStatusBar(notificationView: notificationView, duration: duration, animated: animated)
    }
    
    class func closeNotificationFromTopOfStatusBar(notificationView: UIView) {
        SwiftOverlays.closeAnnoyingNotificationOnTopOfStatusBar(sender: notificationView)
    }
    ...
    func removeAllOverlays() -> Void  {
        ...
        SwiftOverlays.removeAllOverlaysFromView(parentView: self.view)
    }
    ...
    func updateOverlayText(text: NSString) {
        SwiftOverlays.updateOverlayText(parentView: self.view, text: text)
    }
    ...
    func updateOverlayProgress(progress: Float) {
        SwiftOverlays.updateOverlayProgress(parentView: self.view, progress: progress)
    }
}
...
public class SwiftOverlays: NSObject {
    ...
    public class func showBlockingWaitOverlay() -> UIView {
        ...
        _ = showCenteredWaitOverlay(parentView: blocker)
    ...
    public class func showBlockingWaitOverlayWithText(text: NSString) -> UIView {
        let blocker = addMainWindowBlocker()
        _ = showCenteredWaitOverlayWithText(parentView: blocker, text: text)
    ...
    public class func showBlockingImageAndTextOverlay(image: UIImage, text: NSString) -> UIView  {
        let blocker = addMainWindowBlocker()
        _ = showImageAndTextOverlay(parentView: blocker, image: image, text: text)
    ...
    public class func showBlockingTextOverlay(text: NSString) -> UIView  {
        let blocker = addMainWindowBlocker()
        _ = showTextOverlay(parentView: blocker, text: text)
    ...
    public class func removeAllBlockingOverlays() {
        ...
        removeAllOverlaysFromView(parentView: window)
    }
    
    // MARK: Non-blocking
    public class func showCenteredWaitOverlay(parentView: UIView) -> UIView {
        return showCenteredWaitOverlay(parentView: parentView, color: UIColor.white)
    }
    
    public class func showCenteredWaitOverlay(parentView: UIView, color: UIColor) -> UIView {
        let ai = UIActivityIndicatorView(activityIndicatorStyle: UIActivityIndicatorViewStyle.whiteLarge)
        ai.startAnimating()
        ai.color = color
        
        let containerViewRect = CGRect(x: 0, y: 0, width: ai.frame.size.width * 2, height: ai.frame.size.height * 2)
        ...
        ai.center = CGPoint(x: containerView.bounds.size.width/2, y: containerView.bounds.size.height/2);
    ...
    public class func showCenteredWaitOverlayWithText(parentView: UIView, text: NSString) -> UIView  {
        ...
        return showGenericOverlay(parentView: parentView, text: text, accessoryView: ai)
    }
    
    public class func showImageAndTextOverlay(parentView: UIView, image: UIImage, text: NSString) -> UIView  {
        let imageView = UIImageView(image: image)
        
        return showGenericOverlay(parentView: parentView, text: text, accessoryView: imageView)
    }
    ...
    public class func showGenericOverlay(parentView: UIView, text: NSString, accessoryView: UIView, horizontalLayout: Bool = true) -> UIView {
        let label = labelForText(text: text)
        var actualSize = CGSize.zero
        
        if horizontalLayout {
            actualSize = CGSize(width: accessoryView.frame.size.width + label.frame.size.width + padding * 3, height: max(label.frame.size.height, accessoryView.frame.size.height) + padding * 2)
            
            label.frame = label.frame.offsetBy(dx: accessoryView.frame.size.width + padding * 2, dy: padding)
            
            accessoryView.frame = CGRect(x: padding, y: (actualSize.height - accessoryView.frame.size.height)/2, width: 20, height: 20)
        } else {
            actualSize = CGSize(width: max(accessoryView.frame.size.width, label.frame.size.width) + padding * 2, height: label.frame.size.height + accessoryView.frame.size.height + padding * 3)
            
            label.frame = label.frame.offsetBy(dx: padding, dy: accessoryView.frame.size.height + padding * 2)
            
            accessoryView.frame = CGRect(x: (actualSize.width - accessoryView.frame.size.width)/2, y: padding, width: 20, height: 20)
        }
        ...
        let containerViewRect = CGRect(x: 0, y: 0, width: actualSize.width, height: actualSize.height)
        
        let containerView = UIView(frame: containerViewRect)
        
        containerView.tag = containerViewTag
        mainView.tag = containerViewTag
        containerView.layer.cornerRadius = cornerRadius
        ...
        parentView.bringSubview(toFront: mainView)
    ...
    public class func showTextOverlay(parentView: UIView, text: NSString) -> UIView  {
        ...
        let actualSize = CGSize(width: label.frame.size.width + padding * 2, height: label.frame.size.height + padding * 2)
    ...
    public class func showProgressOverlay(parentView: UIView, text: NSString) -> UIView  {
        ...
        return showGenericOverlay(parentView: parentView, text: text, accessoryView: pv, horizontalLayout: false)
    }
    ...
    public class func showAnnoyingNotificationOnTopOfStatusBar(notificationView: UIView, duration: TimeInterval, animated: Bool = true) {
        if bannerWindow == nil {
        ...
        bannerWindow!.frame = CGRect(x: 10.0, y: 10.0, width: UIScreen.main.bounds.size.width-10.0, height: notificationView.frame.size.height)
